...
### Git & Commits
- **No commits without explicit approval** - Always ask before committing.
- **Only push to `develop` branch** - Never push directly to `main`. We push to main only after issues stabilize.
- **All fixes must be tested before committing** - Run relevant tests, verify the fix works.
- **No AI attribution** - Never add "made by Claude", "sent by Claude", "Generated by Claude", or similar to commits, comments, PR descriptions, or any output.

### GitHub Issues
- **Work through issues one by one** - Don't jump between multiple issues.
- **Read the entire issue and all replies** - Ignore images (you can't see them reliably). Read all text comments.
- **Never close issues prematurely** - Wait for confirmation from: the issue creator, another user, or the maintainer. Alternatively, wait 3 days with no response or related issues.
- **Never assume an issue is fixed** - Verify it works on our side first.
- **Never tell a GitHub user they're wrong** - Check our code first, verify everything works, then suggest fixes to them.

### Before Making Any Changes
- **Analyze first, change second** - Read and understand all relevant code before editing. Never assume.
- **Find the root cause** - Don't just patch symptoms. Understand WHY something is broken.
- **Explain the issue to the maintainer** - Before fixing, explain what you found and what you plan to do.
- **Get approval** - Wait for the go-ahead before implementing.

### Preventing Regressions
- **Never reintroduce previously fixed issues** - Before changing code, check CHANGELOG.md for related fixes. Understand why the existing code is the way it is.
- **Run naming tests after path/rename changes** - `python test-env/test-naming-issues.py` covers 39 edge cases from past issues.
- **If unsure, ask** - If you don't understand why code exists, ask before removing or changing it.

### User-Cemented Defaults (DO NOT CHANGE)
- **Port 5757** - This is our app port. Changing it breaks every user's setup, reverse proxies, bookmarks, and Docker configs.
- **Data paths (`/data`, `/config`)** - Users have mounted volumes here. Changing breaks persistence.
- **Config/secrets filenames** - `config.json`, `secrets.json`, `library.db` - Users have these. Renaming loses their data.
- **API endpoint paths** - Users may have scripts or integrations calling these.
- **If you think a default needs changing, ASK FIRST** - We can add alternatives, but never remove or change working defaults without explicit approval.

### Forward Only, Never Backwards
- **Never revert or remove features** - Users rely on existing functionality. We don't take things away.
- **Evolve, don't delete** - If something needs improvement, we build on it (like the scan flow evolution). We don't rip it out.
- **Adding is fine, removing is not** - New options, new settings, new capabilities = good. Removing working features = bad.

### No Incomplete Code
- **Never leave "TODO", "FIXME", or "for now" comments with incomplete logic** - Real users depend on this. Incomplete code causes bugs.
- **If you can't finish something properly, don't commit it** - Ask for help or flag it explicitly.
- **Every code path must be complete** - No stubs, no placeholder logic, no "we'll fix this later".

### Security: Secrets & Privacy
- **NEVER expose secrets in code, logs, debug output, or API responses** - This has happened before with debug code. Don't let it happen again.
- **secrets.json is sacred** - API keys, tokens, passwords stay in secrets.json and NEVER appear in:
  ...
  - Bug reports
  - GitHub comments
- **Sanitize user paths** - Don't expose full filesystem paths (e.g., `/home/username/...`) in logs or error messages. Use relative paths or placeholders like `[library]/...`
- **Test with real secrets in mind** - Before committing, ask: "Could this expose my API keys or the user's private paths?"
...
- `app.py`: main Flask app (~10K lines monolith), worker thread, scanning/AI logic, API routes.
...
- `test-env/test-naming-issues.py` - 39 tests covering GitHub issues (naming edge cases, template cleanup, multibook detection)
- `test-env/test-audio-tagging.py` - Audio metadata embedding tests

Run directly with: `python test-env/test-naming-issues.py`
...
For naming/path logic changes, run the naming tests:
